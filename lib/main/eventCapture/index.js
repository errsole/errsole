...
function httpResponseHandler(payload) {
  if(!payload || !payload.frameworkName) {
    return false;
  }
  getServerSchema(payload.frameworkName)
...
function getServerSchema(name) {
  if (DataExtractionSchema[name]) {
    return Promise.resolve(DataExtractionSchema[name]);
  }
  var data = {
    'framework_name': name,
    'framework_version': '',
  ...
  .then(function(result) {
    if(Object.keys(result).length === 0) {
      console.error('failed to load NODE_FRAMEWORK_KEYS');
    } else {
      appConfig.setNodeFrameworkKeys(name, result);
      DataExtractionSchema[name] = result;
      return result;
    }
  }).catch(function(err) {
    console.error('failed to load NODE_FRAMEWORK_KEYS');
